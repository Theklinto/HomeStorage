<template>
    <DefaultLayout
        :header-label="existingLocation ? t('location.editLocationHeader') : t('location.createLocationHeader')"
        :is-loading="isLoading" :show-error="!!error">
                    ...
                    <div class="container-fluid d-flex flex-column px-4 gap-4">
                        <div class="row">
                            <div class="col-12 d-flex justify-content-center">
                                <input ref="imageInput" type="file" accept="images/*,android/force-camera-workaround"
                                    class="d-none" @change="onFileSelected" />
                                <Button class="w-100 ratio ratio-1x1 position-relative" outlined severity="secondary"
                            ...
                            <div class="col-12">
                                <Fluid>
                                    <FloatLabel variant="on">
                                        <InputText required :minlength="3" v-model="locationModel.name"
                                            :class="{ 'p-invalid': locationModel.name.length > 0 && locationModel.name.length < 3 }" />
                                        <label>{{ t("common.nameFieldLabel") }}</label>
                                    </FloatLabel>
                                </Fluid>
                                <Message class="mt-2" v-if="
                        ...
                        <div class="row mt-5">
                            <div class="col-12">
                                <Button :loading="isLoading" type="submit" fluid
                                    :label="existingLocation ? t('common.updateBtnLabel') : t('common.createBtnLabel')" />
                            </div>
                        </div>
                    </div>
                </fieldset>
...
async function updateLocation() {
    ...
    try {
        ...
        toast.add({ severity: "success", summary: t("location.updateSuccessToastSummary"), life: 3000 });
        router.back();
    }
    catch {
        toast.add({ severity: "error", summary: t("location.locationUpdateErrorToastSummary"), life: 3000 });
    } finally {
        isLoading.value = false;
    }
}
